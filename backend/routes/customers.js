...
router.post('/', async (req, res) => {
  try {
    ...
    const customer = new Customer({
      userId: req.user.email, id: 'C' + uuidv4().slice(0, 8).toUpperCase(),
...
router.delete('/:id', async (req, res) => {
  try {
    ...
    if (permanent === 'true') {
      ...
      const hidden = await Customer.findOneAndUpdate({ userId: req.user.email, id: req.params.id }, { visible: 'false' }, { new: true });
